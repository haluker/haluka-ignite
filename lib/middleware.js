...
module.exports = (express, httpConfig) => {
    ...
    express.use(bodyParser.urlencoded({ extended: true, limit: _.get('post.limit', httpConfig , '2mb') }))

    // trust first proxy
    if (_.get('trustProxy', httpConfig, true))
        express.set('trust proxy', 1) 
        
    // Multipart Parser (busboy-body-parser)
    express.use(busboyParser(_.get('uploads', httpConfig, {})))
    
    // Static Files
    express.use(_express.static('public'))

    // Always wear a helmet
    express.use(helmet(_.get('helmet', httpConfig, { })))

    // Session
    SessionSetup(express, httpConfig.sessionSecret)

    // Gzip compression
    if (_.get('gzip', httpConfig, true))
