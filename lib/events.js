...
Event.on('Http.Error.*', async function (event, err, req, res) {
	...
	if (config('app.env') === 'production') {
		res.format({
			html: () => res.send(Errors[code]),
			json: () => res.json({ status: "error", code: code, message: err.message })
		})
	} else {
		res.format({
			html: () =>res.send(await new Youch(err, req).toHTML()),
			json: () => res.json(await new Youch(err, req).toJSON())
		})
	}
})
